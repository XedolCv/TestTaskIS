...
2. Web-API на .Net Core 6 с использованием EF Core

Tак как по срокам было оговорено 1-2 дня ветка master это то что было сделано в срок, но из за того что срок выполнения истек в пятницу вечером,
то в субботу тестовое задание ещё не посмотрели, на следующий день доделал некоторые моменты в ветке new_features, а именно авторизация пользователя
на Middleware не только по правам , но и по модулям, т.е. на конкретный модуль свой список прав и добавил описания методов в Swagger. Было бы не честно с моей 
стороны выдавать 3-х дневную работу за 2-х дневную , поэтому для чистоты все доделывания в отдельной ветке.

Для удоного просмотра и тестирования при запуске запускается Swagger UI что бы проверить доступные контроллеры и запросы и тут же 
их протестировать. 
